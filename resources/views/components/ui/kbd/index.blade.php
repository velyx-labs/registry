...
@php
    ...
    $keyList = is_string($keys) ? preg_split('/[\s+]+/', $keys) : $keys;
@endphp
<span {{ $attributes->merge(['class' => 'inline-flex items-center gap-0.5 sm:gap-1']) }}>
    @foreach($keyList as $index => $key)
            ...
            @endphp
        <kbd class="inline-flex items-center justify-center {{ $sizeClasses }} {{ $variantClasses }} select-none whitespace-nowrap rounded font-mono font-medium leading-none">
                @if($icon)
