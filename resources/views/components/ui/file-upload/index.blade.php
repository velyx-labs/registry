...
>
    @if($label)
        <label class="mb-2 block text-sm font-semibold text-foreground" data-test="file-upload-label">
            {{ $label }}
        </label>
    @endif
    ...
    >
        ...
        <label
            for="{{ $inputId }}"
            class="flex cursor-pointer flex-col items-center justify-center px-6 py-8"
        ...
        >
            <div class="flex flex-col items-center justify-center text-center">
                ...
                <p class="text-xs text-muted-foreground" data-test="file-upload-hint">
                    {{ $description }}
                    @if($maxSize)
                        <span class="mt-1 block">(Max size: {{ $maxSize }})</span>
                    @endif
                </p>
            </div>
        </label>
        ...
        >
            <div class="space-y-2">
                <template x-for="(fileName, index) in fileNames" :key="index">
                    <div class="bg-muted flex items-center justify-between rounded-lg p-2" data-test="file-name-item">
                        <div class="flex items-center gap-2">
                            <svg class="h-5 w-5 text-muted-foreground" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
                            </svg>
                            <span class="max-w-xs truncate text-sm text-foreground" x-text="fileName" data-test="file-name"></span>
                        </div>
                        <button
                            type="button"
                            @click.prevent="
